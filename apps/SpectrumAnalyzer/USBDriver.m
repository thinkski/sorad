...
- (void)reader:(id)anObject
{
    ...
    kr = (*usbInterfaceInterface)->CreateInterfaceAsyncEventSource(
    ...
    for (i = 0; i < kNumberInternalBuffers; i++) {
        ...
        if (kr != kIOReturnSuccess) {
            statusMessage = @"Error";
            NSLog(@"ReadPipeAsync() error. (code 0x%08x)", kr);
            break;
        }
    }
...
{
    // Index of byte indicating end-of-configuration in response message
    const unsigned int EP1IN_DONE_IDX  = 0;
    
    // Index of byte indicating configuration error in response message
    const unsigned int EP1IN_ERROR_IDX = 1;
	...
	NSString *filename = [[NSBundle mainBundle] pathForResource:@"bitstream"
                                                         ofType:@"bit"];
	
	// Open resource file
	if (!(fp = fopen([filename cStringUsingEncoding:NSASCIIStringEncoding],
                     "rb"))) {
		NSLog(@"%s: fopen() failed", __func__);
		return -1;
	}
	...
	while (!feof(fp)) {
		...
		UInt32 n = fread(buf, sizeof(UInt8), kEndpoint1Size, fp);
			
		// Write buffer to device
		IOReturn kr = (*iref)->WritePipe(iref, kCtrlOutputPipeRef, buf, n);
			
		// Verify that write was successful
		if (kr != kIOReturnSuccess) {
			NSLog(@"%s: WritePipe(): failed", __func__);
			return -1;
		}
			
		// Read feedback signal
		n = kEndpoint1Size;
		kr = (*iref)->ReadPipe(iref, kCtrlInputPipeRef, buf, &n);
			
		// Verify that read was successful
		if (kr != kIOReturnSuccess) {
			NSLog(@"%s: ReadPipe(): failed", __func__);
			return -1;
		}
			
        // Check for configuration error
        if (buf[EP1IN_ERROR_IDX] && !buf[EP1IN_DONE_IDX]) {
            NSLog(@"%s: configuration error", __func__);
            return -1;
        }
    }
    
    // Did FPGA configuration succeed (i.e. was DONE pin asserted)?
    if (buf[EP1IN_DONE_IDX]) {
        return 0;
    } else {
        NSLog(@"%s: done pin not asserted", __func__);
    }
...
static void unconfiguredDeviceAdded(void *refCon, io_iterator_t iter)
{
	...
	while ((usbDevice = IOIteratorNext(iter))) {
		...
		if (FAILED(status)) {
			NSLog(@"unconfiguredDeviceAdded(): QueryInterface() failed: Code 0x%08x", (unsigned int)status);
			break;
		}
    ...
    while ((usbDevice = IOIteratorNext(iter))) {
        ...
        do {
            ...
            if (// Assignments
                ...
                [usbDriverInstance setStatusMessage:@"Cannot connect. See console for error."];
                NSLog (@"Error querying interface via plug-in. (Code 0x%08x)", (unsigned int)herr);
                break;
            }
...
static void configuredDeviceRemoved(void *refCon, io_iterator_t iter)
{
    ...
    while ((usbDevice = IOIteratorNext(iter))) {
        ...
        if (kr != kIOReturnSuccess) {
            ...
            [usbDriverInstance setStatusMessage:@"Not connected"];
        }
	}
}
