...
- (id)init
{
	...
	if (self) {
		...
		decibelsPerDivision = 5.0;		// Match IB
		referenceLevel = -50;
...
// Action for changing the center frequency. Should be sent by a circular
// NSSlider. The difference between the slider's current and previous value is
// used. Sends tuning request to hardware via USBDriver.
...
// Method provides scope view with the number of traces to draw
...
- (NSData *)traceData:(ScopeViewGL *)aScopeViewGL index:(NSInteger)aIndex viewWidth:(NSInteger)width
{
	...
	if (hwData) {
		...
		// Convert 16-bit signed samples provided by hardware into
...
// Method provides scope view with the color in which to draw the trace
